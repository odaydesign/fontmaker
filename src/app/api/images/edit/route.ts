...
export async function POST(request: NextRequest) {
  ...
  try {
    const { prompt, referenceImage, count } = await request.json();
    ...
    if (!referenceImage || typeof referenceImage !== 'string') {
      return NextResponse.json(
        { error: 'A reference image is required for style consistency.' },
        { status: 400 }
      );
    }
    ...
    const message =
      typeof error === 'object' && error !== null && 'message' in error
        ? String((error as { message?: string }).message ?? 'Failed to edit image.')
